...
export default function Drawer({
  ...
  const [drawerStackElement, setDrawerStackElement] = useState<any>(null)
  ...
  useEffect(() => {
    ...
    let temp = document?.getElementById?.(DRAWER_STACK_ID)
    if (!temp) {
  ...
  return ReactDOM.createPortal(
    ...
    >
      ...
      >
        <div
          className={`absolute inset-0 ${maskNoBlur ? '' : 'backdrop-filter backdrop-blur'} bg-stack-4/50`}
          onClick={closeDrawer}
        ></div>
      </Transition.Child>
      <Transition.Child
