...
import { isBigInt, isBN, isBoolean, isNumber, isString } from '@/functions/judgers/dateType'
 ...
 * @returns
 */
export function useSort<D extends Record<string, any>[]>(
  sourceDataList: D,
  options?: {
...
function compareForSort(a: unknown, b: unknown): number {
  if (isBN(a) && isBN(b)) {
    const sub = a.sub(b)
    return sub.lt(new BN(0)) ? -1 : sub.gt(new BN(0)) ? 1 : 0
